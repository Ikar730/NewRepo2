...
int[] Reverse(int[] array, int i = 0)
{
    if (i < array.Length / 2)
    {
        int temp = array[i]; // 1 элемент, 2 элемент, 3-й и так далее до половины

        // переписываем элемент с начала массива значением элемента с конца массива
        array[i] = array[array.Length - i - 1];

        // переписываем элемент с конца массива значением элемента с начала массива
        array[array.Length - i - 1] = temp;

        // увеличиваем индекс элемента и вызываем рекурсивно метод
        Reverse(array, ++i);
    }
    return array;
}
...
//test[1] = 3 + test[0] = 4;
//test[0] = test[1] - test[0] = 4 - 1 = 3;
//test[1] = test[1] - test[0] = 4 - 3 = 1;
